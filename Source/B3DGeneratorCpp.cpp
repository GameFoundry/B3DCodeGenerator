...
static std::string GenerateScriptObjectToScriptObjectWrapper(const std::string& indent, const std::string& scriptWrapperType, const std::string& scriptWrapperVariableName, const std::string& scriptObjectVariableName, const VariableTypeInformation& typeInformation, const TypeMappingInformation& typeMappingInformation)
{
	...
	else
	{
		...
		if(isUsingIScriptExportableAPI)
			...
			output << indent << scriptWrapperVariableName << " = (" << scriptBaseType << "*)" << scriptWrapperType << "::ToNative(" << scriptObjectVariableName << ");" << std::endl;
	}
...
static std::string GenerateMethodBodyBlockForArgument(const std::string& parameterName, const VariableBase& parameterInformation, bool isLast, bool returnValue, std::stringstream& preCallActions, std::stringstream& postCallActions)
{
	...
	if(parameterInformation.TypeInformation.TypeCategory == VariableTypeCategory::AsyncOp)
	{
		...
		{
			std::string scriptType = TypeLookup::GetScriptInteropTypeName(asyncOpUnderlyingTypeInformation.GetLastWrappedOrSelfTypeName(),
	...
	{
		const std::string scriptType = TypeLookup::GetScriptInteropTypeName(inputType);
	...
	if (!parameterInformation.TypeInformation.IsArrayOrVector())
	{
		...
		case ExportedClassTypeCategory::Struct:
		{
			...
			else
			{
				if (parameterTypeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Struct && parameterInformation.TypeInformation.IsPostProcessFlagSet(VariablePostProcessFlags::IsStructWrapperUsed))
				{
					std::string scriptType = TypeLookup::GetScriptInteropTypeName(parameterTypeName);
		...
		case ExportedClassTypeCategory::GUIElement:
		{
			const std::string scriptObjectWrapperType = TypeLookup::GetScriptInteropTypeName(parameterTypeName);
		...
		default: // Some resource or game object type
		{
			...
			else
			{
				...
				const std::string scriptObjectWrapperType = TypeLookup::GetScriptInteropTypeName(parameterTypeName, parameterInformation.TypeInformation.IsParameterFlagSet(ParameterFlags::AsResourceRef));
	...
	assert(parameterInformation.TypeInformation.IsArrayOrVector());
	{
		...
		default: // Some object or struct type
			arrayEntryTypeName = TypeLookup::GetScriptInteropTypeName(parameterTypeName, arrayElementTypeInformation.IsParameterFlagSet(ParameterFlags::AsResourceRef));
			break;
		}
...
static std::string GenerateFieldConvertBlock(const std::string& name, const VariableBase& fieldInformation, bool toInterop, std::stringstream& preActions)
{
	...
	if (!fieldInformation.TypeInformation.IsArrayOrVector())
	{
		...
		case ExportedClassTypeCategory::Struct:
			if(fieldInformation.TypeInformation.IsPostProcessFlagSet(VariablePostProcessFlags::IsStructWrapperUsed))
			{
				...
				std::string scriptType = TypeLookup::GetScriptInteropTypeName(fieldTypeName);
		...
		case ExportedClassTypeCategory::GUIElement:
		{
			const std::string scriptType = TypeLookup::GetScriptInteropTypeName(fieldTypeName);
		...
		default: // Some resource or game object type
		{
			std::string scriptType = TypeLookup::GetScriptInteropTypeName(fieldTypeName, fieldInformation.TypeInformation.IsParameterFlagSet(ParameterFlags::AsResourceRef));
			std::string scriptName = "script" + name;
	...
	assert(fieldInformation.TypeInformation.IsArrayOrVector());
	{
		...
		case ::ExportedClassTypeCategory::MonoObject:
			...
			break;
		default: // Some object or struct type
			entryType = TypeLookup::GetScriptInteropTypeName(fieldTypeName, arrayElementTypeInformation.IsParameterFlagSet(ParameterFlags::AsResourceRef));
			break;
		}
...
static std::string GenerateEventCallbackBodyBlockForArgument(const std::string& name, const VariableBase& parameterInformation, std::stringstream& preCallActions)
{
	...
	if (!parameterInformation.TypeInformation.IsArrayOrVector())
	{
		...
		case ExportedClassTypeCategory::Struct:
			{
				const std::string scriptType = TypeLookup::GetScriptInteropTypeName(parameterTypeName);
	...
	assert(parameterInformation.TypeInformation.IsArrayOrVector());
	{
		...
		case ExportedClassTypeCategory::MonoObject:
			...
			break;
		default: // Some object or struct type
			entryType = TypeLookup::GetScriptInteropTypeName(parameterTypeName, arrayElementTypeInformation.IsParameterFlagSet(ParameterFlags::AsResourceRef));
			break;
		}
...
static std::string GenerateClassDeclaration(const ClassInfo& classInfo)
{
	...
	if(isUsingIScriptExportableAPI)
	{
		if(!isModule)
		{
			if(typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Class || typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::ReflectableClass)
				output << "TScriptObjectWrapper<" << wrappedDataType << ", " << interopClassName;
			else
			{
				// TODO - Support other types as well
				errs() << "Unable to generate class for \"" << classInfo.NativeName << "\" as IScriptExportable interface is not supported.";
			}
		}
		else
	...
	{
		if(typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Resource)
			output << "TScriptResource<" << interopClassName << ", " << classInfo.NativeName;
		else if(typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Component)
	...
	if(typeMappingInformation.IsClassType() && !isModule && !isUsingIScriptExportableAPI)
	{
	...
	if (typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Resource && !isUsingIScriptExportableAPI)
	{
	...
	// Handle (if required)
	if (typeMappingInformation.IsClassType() && !isUsingIScriptExportableAPI)
	{
		if (!classInfo.Events.empty())
	...
	// Data member
	if (typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Class && !isModule && classInfo.BaseClassName.empty() && !isBase && !isUsingIScriptExportableAPI)
	{
		output << "\t\t" << wrappedDataType << " mInternal;" << std::endl;
		output << std::endl;
	}
...
static std::string GenerateClassDefinition(const ClassInfo& classInfo)
{
	...
	auto fnGenerateEventHandles = [&classInfo, isUsingIScriptExportableAPI](std::stringstream& stream, const std::string& className)
	{
		const bool isModule = classInfo.IsFlagSet(ClassFlags::IsModule);
	...
	else
	{
		...
		{
			if(!isModule && !isBase && classInfo.BaseClassName.empty())
	...
	if (typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Class && !isUsingIScriptExportableAPI)
	{
		// GetInternal method
		if (isBase || !classInfo.BaseClassName.empty())
		{
	...
	if (typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Resource)
	{
		if(isUsingIScriptExportableAPI)
			...
			output << "\t\tmetaData.ScriptClass->AddInternalCall(\"Internal_GetRef\", (void*)&" << interopClassName << "::InternalGetRef);\n";
	}
	...
	for (auto& methodInfo : classInfo.Constructors)
	{
		...
		if(isUsingIScriptExportableAPI)
			...
			output << "\t\tmetaData.ScriptClass->AddInternalCall(\"Internal_" << methodInfo.InteropName << "\", (void*)&" << interopClassName << "::Internal" << methodInfo.InteropName << ");" << std::endl;
		output << GenerateApiCheckEnd(methodInfo.API);
	}
	...
	if ((typeMappingInformation.IsClassType() && !isModule) || typeMappingInformation.TypeCategory == ::ExportedClassTypeCategory::Resource)
	{
		...
		{
			if(typeMappingInformation.TypeCategory != ExportedClassTypeCategory::GUIElement)
				output << "\tMonoObject* " << interopClassName << "::GetOrCreateScriptObject(const " << wrappedDataType << "& nativeObject)\n";
			else
			{
				output << "\tUNHANDLED CASE\n";
				// TODO - Handle GUI element case - ScriptObjectWrapper needs to support raw pointers
			}
			...
			output << "\t\t\treturn scriptObjectWrapper->GetScriptObject();\n";

			output << ctorParamsInit.str();
			output << "\t\tMonoObject* scriptObject = sInteropMetaData.ScriptClass->CreateInstance(\"" << ctorSignature.str() << "\", ctorParams);\n";
			output << "\t\tB3DNew<" << interopClassName << ">(nativeObject, scriptObject);\n";
			output << "\t\treturn scriptObject;\n";
			...
			if (!isCallback)
			{
				if (isStatic)
				{
					if(isUsingIScriptExportableAPI)
						...
						output << "\t\t" << eventInfo.NativeName << "Conn = ";
					output << classInfo.NativeName << "::" << eventInfo.NativeName << ".Connect(&" << interopClassName << "::" << eventInfo.InteropName << ");" << std::endl;
				}
				else if (isModule)
				{
					if(isUsingIScriptExportableAPI)
						...
						output << "\t\t" << eventInfo.NativeName << "Conn = ";
					output << classInfo.NativeName << "::Instance()." << eventInfo.NativeName << ".Connect(&" << interopClassName << "::" << eventInfo.InteropName << ");" << std::endl;
				}
			}
			else
			{
				if (isStatic)
					output << classInfo.NativeName << "::" << eventInfo.NativeName << " = &" << interopClassName << "::" << eventInfo.InteropName << ";" << std::endl;
				else if (isModule)
					output << classInfo.NativeName << "::Instance()." << eventInfo.NativeName << " = &" << interopClassName << "::" << eventInfo.InteropName << ";" << std::endl;
			}
		}
...
static std::string GenerateStructDeclaration(const StructInfo& structInfo)
{
	...
	std::string interopClassName = TypeLookup::GetScriptInteropTypeName(structInfo.NativeName);
...
{
	const TypeMappingInformation typeMappingInformation = TypeLookup::GetNativeToScriptTypeMapping(structInfo.NativeName);
	const std::string interopClassName = TypeLookup::GetScriptInteropTypeName(structInfo.NativeName);
